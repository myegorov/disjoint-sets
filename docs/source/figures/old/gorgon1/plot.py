...
def plot_scatter(data, title, xlabel, ylabel, fname):
    ...
    colors = itertools.cycle(['b', 'g', 'r', 'lavender', 'm', 'crimson', 'k', 'plum'])
    markers = itertools.cycle(['+', 'o', 'x', '.', '|', 'v', '_', 's', '*'])

    for algo in data.keys():
        color = next(colors)
        marker = next(markers)

        plt.scatter(data[algo]['x'], data[algo]['y'], s=60,
                c=color, marker=marker, label=algo)
    plt.grid()
    plt.legend(loc='center left', bbox_to_anchor=(1, 0.5),
                  ncol=1, fancybox=True, shadow=True,
                  scatterpoints = 1)
    fig.savefig(os.path.join(DOCDIR,fname), bbox_inches = 'tight')
    #plt.show()

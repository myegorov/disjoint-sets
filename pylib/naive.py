...
def lcs_naive(seq1, seq2):
    ...
    return _lcs_naive(seq1, seq2, len(seq1)-1, len(seq2)-1, "")
...
def _lcs_naive(seq1, seq2, i, j, lcs):
    ...
    if i < 0 or j < 0:
        ...
        if seq1[i] == seq2[j]:
            return _lcs_naive(seq1, seq2, i-1, j-1, seq1[i] + lcs)
        else:
            return max(_lcs_naive(seq1, seq2, i-1, j, lcs),
...
if __name__ == "__main__":
    ...
    print("[%0.7fs] %s(%d) -> %d recursive calls"
            %(elapsed, name, len(sequence_1), registry['_lcs_naive']))

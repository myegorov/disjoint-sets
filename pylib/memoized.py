...
def tabulate_lcs(seq1, seq2, *args):
    ...
    Args:
        ...
        seq2 (string):  another random string
                sequence like seq1
    Returns:
        table of LCS lengths (int): so-called table
                c in Figure 15.8 in CLRS
...
def _tabulate_lcs(seq1, seq2, i, j, lcs_table):
    """Recursive solution with memoization to LCS
    problem. See CLRS ex. 15.4-3.
    ...
    Args:
        ...
        lcs_table (2D list):   a matrix of LCS length
                            for [i, j] prefix
    Returns:
...
def _reconstruct_lcs(seq1, seq2, lcs_table, char, i, j,\
    ...
    # else consider general case
        ...
        if (seq1[i] == seq2[j]):
            lcs_arr[char] = seq1[i]
            return _reconstruct_lcs(seq1, seq2, \
                    lcs_table, char-1, i-1, j-1, lcs_arr)

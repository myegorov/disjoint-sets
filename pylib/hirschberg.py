...
__date__ = "2016-05-01 Sun 07:50 PM"
...
def tabulate_lcs(seq1, seq2, *args):
    ...
    # select min(|seq1|, |seq2|) for vector
    # storage
    if len1 < len2:
        seq1, seq2 = seq2, seq1
        len1, len2 = len2, len1
    ...
    # the matrix can be reduced to a min(m,n)+1
    # vector as the dynamic programming approach
    # only needs the
...
@log_recursion
def _tabulate_lcs(seq1, seq2, i, j, lcs_vector):
    """Iterative Hirschberg dynamic programming
    solution to LCS problem. See Hirschberg's ALG B.
    ...
    Args:
        ...
        j (int):        number of columns in LCS table
                            (=len(seq2) + 1)
        lcs_vector (1D list):   a vector of LCS length
                    for [i-1, j-1] prefix, as in ALG B
    Returns:
...
def reconstruct_lcs(seq1, seq2):
    ...
    lcs_arr = _reconstruct_lcs(seq1, seq2, m, n)

    lcs = "".join(lcs_arr)
    return lcs

@log_recursion
def _reconstruct_lcs(seq1, seq2, m, n):
    ...
    if n == 0:
        ...
        _tabulate_lcs(seq1[:mid], seq2, \
                mid+1, n+1, lcs_vector_1)
        _tabulate_lcs(seq1[:mid-1:-1], seq2[::-1],
                m-mid+1, n+1, lcs_vector_2)

        sums = [lcs_vector_1[i] + lcs_vector_2[n-i] \
                for i in range(len(lcs_vector_1))]

        k = sums.index(max(sums))

        C1 = _reconstruct_lcs(seq1[:mid], seq2[:k], \
                mid, min(k, n))
        C2 = _reconstruct_lcs(seq1[mid:], seq2[k:], \
                m-mid, n-min(k, n))
        C1.extend(C2)
        return C1
...
def size_lcs(lcs_vector):
    ...
    Args:
        lcs_vector (1D list):   a vector of LCS
            length for [i, j] prefix
    Returns:
...
if __name__ == "__main__":
    ...
    # reconstruct LCS
    name, elapsed, memlog, lcs = reconstruct_lcs(sequence_1, sequence_2)
    lcs_length = len(lcs)
    print("\n(an) LCS: ", lcs)
    print("""[%0.7fs] %s(%d) -> %d recursive calls to _algC,
                %d to _algB"""
            %(elapsed, name, lcs_length, \
                    registry['_reconstruct_lcs'], registry['_tabulate_lcs']))
